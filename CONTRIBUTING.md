# Contributing to UsingOpen

UsingOpen is an open-source project and we welcome contributions from the community.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the UsingOpen team.
...
4. **Security** is always important. If you have a security concern, please do not open an issue. Instead, please open a GitHub Security Advisory on this fork or contact us at [support@usingopen.com](mailto:support@usingopen.com).
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core UsingOpen project, but are maintained by the UsingOpen team.
...
Changes to the core UsingOpen project are released through the `master` branch. When a PR is merged into `master`, a new version of the package is published to Docker and GitHub Container Registry under the `latest` tag.
...
Changes to the desktop app are downstream of the core UsingOpen project. Releases of the desktop app are published at the same time as the core UsingOpen project. Code from the core UsingOpen project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core UsingOpen project is **not** part of the core UsingOpen project, but is maintained by the UsingOpen team.

## 🔌 Criteria for New LLM Providers

To ensure the long-term maintainability of UsingOpen and prevent repository bloat, we enforce a vetting process for adding new third-party LLM provider integrations. 
...
> These guidelines apply strictly to unsolicited community or third-party startup contributions. If you are an ecosystem, silicon, or cloud hardware partner engaging directly with the UsingOpen maintainers on a co-developed integration, proof-of-concept, or native optimization project, this vetting process is not applicable.
...
We cannot act as a discovery or marketing engine for early-stage startups. To qualify for codebase inclusion, a provider must demonstrate an active, existing user base who would benefit from UsingOpen's functionality. Any of the following criteria are acceptable:
...
By contributing to UsingOpen (this repository), you agree to license your contributions under the MIT license.
