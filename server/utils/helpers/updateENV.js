...
const KEY_MAPPING = {
  ...
  */
  ...
  OpenRouterServiceTier: {
    ...
    checks: [
      (input) => {
        // Mirrors OpenRouter service tiers (was OpenRouterLLM.SERVICE_TIERS).
        return ["auto", "default", "fast", "flex", "priority", "scale"].includes(
          input
        )
          ? null
          : `Invalid service tier. Must be one of: auto, default, fast, flex, priority, scale.`;
      },
    ],
  },
...
async function validDockerizedUrl(input = "") {
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the UsingOpen container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
