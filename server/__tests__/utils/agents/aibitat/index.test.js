...
describe("AIbitat.getProviderForConfig providerSlug wiring", () => {
  ...
  test("stamps the instance with the UsingOpen slug it was built from", () => {
    const aibitat = new AIbitat({ provider: "ollama", model: "llama3:latest" });
    const provider = aibitat.getProviderForConfig({
      provider: "ollama",
      model: "llama3:latest",
    });

    // The slug must be the pricing-map key ("ollama"), not the class name
    // ("OllamaProvider") that goes into the metrics `provider` field.
    expect(provider.providerSlug).toBe("ollama");
  ...
  test("re-routing to a different slug stamps the new delegate's slug", () => {
    ...
    expect(first.providerSlug).toBe("ollama");
